...
from schemas import Cliente, Direccion, Factura, Pedido, Producto

app = FastAPI(title="TiendaOnline", version="1.0.0")

# Base de datos simulada en memoria
productos_db: List[Producto] = []
direccions_db: List[Direccion] = []
...
@app.get("/direccions", response_model=List[Direccion])
def get_direccions():
    """Listar todos los direccions"""
    return direccions_db


@app.get("/direccions/{direccion_id}", response_model=Direccion)
def get_direccions_direccion_id(direccion_id: int):
    """Obtener un direccion por ID"""
    item = next((i, x) for i, x in enumerate(direccions_db) if i == direccion_id)
    if not item:
        raise HTTPException(status_code=404, detail="Direccion no encontrado")
    return item


@app.post("/direccions", response_model=Direccion, status_code=201)
def post_direccions(data: Direccion):
    """Crear un nuevo direccion"""
    direccions_db.append(data)
    return data


@app.put("/direccions/{direccion_id}", response_model=Direccion)
def put_direccions_direccion_id(direccion_id: int, data: Direccion):
    """Actualizar un direccion existente"""
    if direccion_id >= len(direccions_db):
        raise HTTPException(status_code=404, detail="Direccion no encontrado")
    direccions_db[direccion_id] = data
    return data


@app.delete("/direccions/{direccion_id}", response_model=dict)
def delete_direccions_direccion_id(direccion_id: int):
    """Eliminar un direccion"""
    if direccion_id >= len(direccions_db):
        raise HTTPException(status_code=404, detail="Direccion no encontrado")
    direccions_db.pop(direccion_id)
